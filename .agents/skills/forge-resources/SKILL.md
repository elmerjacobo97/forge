...
description: Manage Forge resources via the forge-cli binary (create, list, get, update, delete). Use when the user asks to save a note, prompt, config, or snippet resource, list resources, update or delete a resource, or run forge-cli resource commands against InsForge.
---
...
Resources sync to the InsForge `snippets` table and RLS policies used by the web `/resources` tool. Auth is per-user; there is no offline mode.
...
- Kinds: `note`, `prompt`, `config`, `snippet`.
