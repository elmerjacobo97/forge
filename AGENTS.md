...
- Keep the CLI under `apps/cli/src/` (commands, Appwrite client, bookmark/project/dev-board services, validation). Docs: `apps/cli/README.md`. Agent skills: `.claude/skills/forge-bookmarks/`, `.agents/skills/forge-bookmarks/`, `.claude/skills/forge-projects/`, `.agents/skills/forge-projects/`, `.claude/skills/forge-tickets/`, `.agents/skills/forge-tickets/`.
...
- Config/session: `~/.forge/config.json` and `~/.forge/session.json` after `forge-cli init` and `forge-cli login`. Config includes bookmarks + Dev Board table IDs (`devBoardProjectsTableId`, `devBoardTicketsTableId`, `devBoardEventsTableId`, `devBoardTimeEntriesTableId`).
