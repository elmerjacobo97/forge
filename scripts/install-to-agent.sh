...
skill_dirs() {
  find "$SKILLS_REPO/skills/forge" -name SKILL.md -not -path '*/node_modules/*' -not -path '*/deprecated/*' -print0 |
    xargs -0 -n1 dirname
}
...
case "$AGENT" in
  ...
  codex)
    ...
    BLOCK_START="<!-- forge-skills:start -->"
    BLOCK_END="<!-- forge-skills:end -->"

    tmp_block="$(mktemp)"
    {
      echo "$BLOCK_START"
      echo "## Skills (forge-skills)"
      echo
      echo "Installed from https://github.com/elmerjacobo97/forge. Each entry is a workflow you can invoke by reading the linked file and following its steps."
      echo
      skill_dirs | while IFS= read -r src; do
        name="$(basename "$src")"
        desc="$(fm "$src/SKILL.md" description)"
