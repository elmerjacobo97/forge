...
import type { ForgeConfig, ForgeSession } from "@forge/core";
  ...
  return {
    ...
    refreshToken:
      typeof record.refreshToken === "string" && record.refreshToken.length > 0
        ? record.refreshToken
        : (fallbackRefreshToken ?? requiredString(record, "refreshToken", context)),
  };
}
...
export async function requireConfig(): Promise<ForgeConfig> {
  ...
  if (!config) {
    throw new Error("No config found. Run: forge-cli init --from-web-env  (or forge-cli init)");
  }
  return config;
}
...
export function createClient(config: ForgeConfig, accessToken?: string): InsForgeClient {
  return createForgeClient(config, accessToken);
}
