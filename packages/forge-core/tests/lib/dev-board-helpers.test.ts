...
describe("applyMoveTicket", () => {
  ...
  it("starts timer when entering validation", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-07-11T15:06:00.000Z"));
    const moving = ticket("moving", "todo", 0);

    const moved = applyMoveTicket(moving, "validation", [moving]);
  ...
  it("keeps the timer running between in_progress and validation and stops at review", () => {
    vi.useFakeTimers();
    ...
    const toValidation = applyMoveTicket(running, "validation", [running]);
    ...
    const toReview = applyMoveTicket(toValidation, "review", [toValidation]);
...
describe("resolveUpdateEventType", () => {
  it("maps column changes to started / completed / moved", () => {
    ...
    expect(resolveUpdateEventType(base, { ...base, column: "in_progress" })).toBe("started");
    expect(resolveUpdateEventType(base, { ...base, column: "validation" })).toBe("started");
    expect(resolveUpdateEventType(base, { ...base, column: "done" })).toBe("completed");
    expect(resolveUpdateEventType(base, { ...base, column: "review" })).toBe("moved");

    const running = ticket("t1", "in_progress", 0, {
      timerStartedAt: "2026-07-11T15:00:00.000Z",
    });
    expect(resolveUpdateEventType(running, { ...running, column: "validation" })).toBe("moved");
  });
  ...
  it("returns null when nothing timer/column related changed", () => {
    const base = ticket("t1", "todo", 0);
    expect(resolveUpdateEventType(base, { ...base, title: "Renamed" })).toBeNull();
  });
});
